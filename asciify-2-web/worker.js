...
self.addEventListener("message", (event) => {
  ...
  if (message.type === "search") {
    ...
    if (outputMode === "color-search") {
      result = colorBeamSearch(message.target, message.lineIndex);
    } else if (outputMode === "post-color") {
      result = postColorSearch(message.target, message.lineIndex);
    } else {
      result = monochromeBeamSearch(message.target, message.lineIndex);
    }
...
function postColorSearch(target, lineIndex) {
  const monochromeResult = monochromeBeamSearch(rgbToGray(target), lineIndex);
  return {
    ...monochromeResult,
    colors: fitColorsToText(target, monochromeResult.text),
  };
}

function rgbToGray(target) {
  const gray = new Uint8Array(lineWidth * glyphHeight);
...
function fitColorsToText(target, text) {
  const errorWeights = new Float32Array(lineWidth * glyphHeight);
  for (let index = 0; index < errorWeights.length; index++) {
    const offset = index * 3;
    const luminance =
      0.299 * target[offset] + 0.587 * target[offset + 1] + 0.114 * target[offset + 2];
    errorWeights[index] = 1 + foregroundWeight * (1 - luminance / 255);
  }

  const glyphIndexes = [...text].map((char) => glyphs.findIndex((glyph) => glyph.char === char));
  const colors = [];
  let width = 0;

  for (let index = 0; index < glyphIndexes.length; index++) {
    const glyphIndex = glyphIndexes[index];
    const nextGlyphIndex = glyphIndexes[index + 1];
    const start = pythonRound(width);
    let advance = advances[glyphIndex];
    if (nextGlyphIndex !== undefined) {
      const pairAdvance = pairAdvances[glyphIndex * glyphCount + nextGlyphIndex];
      advance = pairAdvance >= 0 ? pairAdvance : advance;
    }
    width += advance;
    const end = pythonRound(width);
    colors.push(fitSegmentColor(target, errorWeights, glyphIndex, start, end - start));
  }

  return colors;
}

function fitSegmentColor(target, errorWeights, glyphIndex, x, width) {
  const glyph = glyphs[glyphIndex];
  const numerator = [0, 0, 0];
  let denominator = 0;
  ...
  for (let y = 0; y < glyphHeight; y++) {
    ...
    for (let offset = 0; offset < width; offset++) {
      ...
      const alpha = 1 - glyphPixel / 255;
      const weight = errorWeights[targetIndex];
      denominator += alpha * alpha * weight;
      const rgbIndex = targetIndex * 3;
      for (let channel = 0; channel < 3; channel++) {
        const desired = target[rgbIndex + channel] / 255 - (1 - alpha);
        numerator[channel] += alpha * desired * weight;
  ...
  return numerator.map((value) => {
    const ideal = denominator > 1e-12 ? value / denominator : 0;
    const normalized = Math.max(0, Math.min(1, ideal));
    return Math.round(Math.round(normalized * (paletteLevels - 1)) * step);
  });
}
